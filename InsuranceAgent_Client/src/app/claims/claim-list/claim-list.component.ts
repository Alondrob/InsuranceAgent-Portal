...
@Component({
  selector: 'app-claim-list',
  standalone: true,
  imports: [CommonModule, MatIconModule, MatDialogModule, FilterDialogComponent],
  templateUrl: './claim-list.component.html',
  styleUrls: ['./claim-list.component.css']
})
export class ClaimListComponent implements OnInit {
  ...
  constructor(private claimService: ClaimService, public dialog: MatDialog, private el: ElementRef) {}
  ...
  openFilterDialog(column: keyof Claim, event: MouseEvent, columnIndex: number): void {
    const target = event.target as HTMLElement;
    const rect = target.getBoundingClientRect();
    const columnWidth = rect.width;
    const dialogLeft = rect.left + window.scrollX - columnWidth / 2; // Adjust position to center under the clicked column

    const dialogRef = this.dialog.open(FilterDialogComponent, {
      width: `${columnWidth * 3}px`, // Span three columns
      data: { filterValue: '' },
      position: { top: `${rect.bottom + window.scrollY}px`, left: `${dialogLeft}px` }
    });
  ...
  applyFilter(column: keyof Claim, filterValue: string): void {
    this.filteredClaims = this.claims.filter(claim =>
      claim[column].toString().toLowerCase().includes(filterValue.toLowerCase())
    );
  }
}
